...
func RsaPublicEncrypt(data, publicKey string) (string, error) {
	...
	err = pubKeyIO(pubKey, bytes.NewReader([]byte(data)), output, true)
	if err != nil {
		return "", err
	}
...
func RsaPriKeyEncrypt(data, privateKey string) (string, error) {
	...
	output := bytes.NewBuffer(nil)
	err = priKeyIO(prikey, bytes.NewReader([]byte(data)), output, true)
	if err != nil {
		return "", err
	}
...
func RsaPublicDecrypt(data, publicKey string) (string, error) {
	...
	output := bytes.NewBuffer(nil)
	err = pubKeyIO(pubkey, bytes.NewReader([]byte(databs)), output, false)
	if err != nil {
		return "", err
	}
...
func RsaPriKeyDecrypt(data, privateKey string) (string, error) {
	...
	output := bytes.NewBuffer(nil)
	err = priKeyIO(prikey, bytes.NewReader(databs), output, false)
	if err != nil {
		return "", err
	}
...
// 设置私钥
func getPriKey(privatekey string) (*rsa.PrivateKey, error) {
	var prikey = `
-----BEGIN 私钥-----
%s
-----END 私钥-----
